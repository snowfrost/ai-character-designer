...
生成后把完整提示词（中文 / English / 负向 / 三视图）展示给用户**逐条确认**：

- 确认无误 → 进入出图（第 7 步）
- 要改 → 回改角色卡对应字段 → 重新渲染 → **再次确认**，循环直到确认
- 只想要某条/某视图 → 用 `--models` / `--views` 限定后出图
...
- **2026-08-22 · v1.5** — 新增「用户确认提示词」环节：生成后先把完整提示词（中英/负向/三视图）展示给用户确认，确认无误才出图；`generate.py --generate` 出图前打印提示词摘要并询问（`--yes` 跳过，自动化用）。
